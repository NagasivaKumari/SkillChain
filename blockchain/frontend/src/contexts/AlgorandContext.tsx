...
interface AlgorandContextType {
    ...
    createAsset: (name: string, unit: string) => Promise<number>;
...
export const AlgorandProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const createAsset = async (name: string, unit: string) => {
        if (!account) throw new Error('No account connected');
        const assetId = await AlgorandService.createAsset(account, name, unit);
        await updateAccountInfo(account.addr.toString());
        return assetId;
    };
